package com.usr.secretconfig.client;
...
import com.usr.secretconfig.common.util.HttpResult;
...
public final class SecretConfigService {
    ...
    static Map<String, String> secretConfig(String appName, int configType) {
        HttpResult<Map<String, String>> rlt = httpGet(
            SECRET_CONFIG_DOMAIN + String.format("/secret-config/config?appName=%s&configType=%d", appName, configType),
            new TypeReference<>() {});
        if (!rlt.isSuccess()) {
    ...
    static String rootKey() {
        HttpResult<List<String>> rtkRlt =
            httpGet(SECRET_CONFIG_DOMAIN + "/secret-config/allRtk", new TypeReference<>() {});
        if (!rtkRlt.isSuccess()) {
